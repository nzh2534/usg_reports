...
# zip_old = requests.get('https://prod-grants-gov-chatbot.s3.amazonaws.com/extracts/GrantsDBExtract' + prior + 'v2.zip')
# zip_new = requests.get('https://prod-grants-gov-chatbot.s3.amazonaws.com/extracts/GrantsDBExtract' + today + 'v2.zip')
...
agency_list_nav = ["USAID", "USDS", "USDA"]
message_final = ""
...
for agency, cfda_num in zip(agency_list, cfda_num_list):
    ...
    # --- New Content ---
    added_df = refine_df(added_df, countries)
    removed_df = refine_df(removed_df, countries)
    df_new_final = refine_df(df_new_final, countries)
    df_old_final = refine_df(df_old_final, countries)
    # --- End Content ---
    ...
    comparison_df.to_csv(agency+"_comparison.csv")

    # if added_df.empty and removed_df.empty and comparison_df.empty:
    #     agency_list_nav.remove(agency)
    # else:
    agency_title_orig = "<div id='" + agency + "'><hr class='solid'><br><h1 style='text-align: center;'>" + agency.center(100) + "</h1></div>"
    exists_check = False

    if not added_df.empty:
        exists_check = True
        message += "<br><br><b>The following opportunities have been added to Grants.Gov:</b><br>"
        index = 1
    ...
    if exists_check:
        message_final += agency_title_orig + message
    else:
        agency_list_nav.remove(agency)
...
for agency in agency_list_nav:
    nav += '<a href="#' + agency + '">' + agency + '</a>'
nav += '</div></div>'


with open("grants_gov_weekly_report_" + today + ".html", "w") as file:
    file.write(head_str +"<body style='background-color: #dae19a'>" + nav + message_final + "</body>")
